...
def ensure_test_database():
    ...
    default_url = settings.database_url.replace("/conversa_test", "/postgres")
    ...
    result = conn.execute(
        text("SELECT 1 FROM pg_database WHERE datname = 'conversa_test'")
    )
    if not result.scalar():
        logger.debug("Creating test database...")
        conn.execute(text("CREATE DATABASE conversa_test"))
    else:
...
def drop_test_database():
    ...
    conn.execute(text("""
        ...
        WHERE datname = 'conversa_test' AND pid <> pg_backend_pid()
    """))

    # Drop the test database
    conn.execute(text("DROP DATABASE IF EXISTS conversa_test"))
    conn.close()
    engine.dispose()
